...
function mapVariant(v: DbVariant): Variant {
  return { label: v.name, price: Number(v.price), inStock: v.active && Number(v.stock_quantity) > 0, image: v.image_url ?? undefined, stockQuantity: Math.max(0, Number(v.stock_quantity)) } as Variant & { image?: string; stockQuantity: number };
}
...
export async function getCatalogue(): Promise<Catalogue> {
  ...
  try {
    ...
    const productIds = dbProducts.map((p) => p.id); let variants: DbVariant[] = [];
    if (productIds.length) {
      ...
      if (error) throw error; variants = (data ?? []) as DbVariant[];
    }
    const categorySlugById = new Map(dbCategories.map((c) => [c.id, c.slug]));
    const categories = dbCategories.map((c) => { const m = categoryMetadata(c.slug); return { slug: c.slug, name: c.name, blurb: c.description ?? m?.blurb ?? "", image: c.image_url || m?.image || "", subcategories: m?.subcategories ?? [] }; });
