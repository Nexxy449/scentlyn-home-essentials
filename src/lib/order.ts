...
export type PaymentMethod = { id: PaymentMethodId; name: string; description: string; live: boolean };
...
export function makeReference() { return `SCN-${Date.now().toString().slice(-6)}`; }
export function saveOrder(order: Order) { try { window.sessionStorage.setItem(KEY, JSON.stringify(order)); } catch { /* ignore */ } }
export function readOrder(): Order | null { try { const raw = window.sessionStorage.getItem(KEY); return raw ? (JSON.parse(raw) as Order) : null; } catch { return null; } }
