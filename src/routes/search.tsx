...
function SearchPage() {
  ...
  const results = query
    ? products.filter((p) => [p.name, p.brand, p.category, p.subcategory, p.short, p.description, ...p.variants.map((v) => v.label)]
        .filter(Boolean)
    ...
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
      ...
      <form
        onSubmit={(e) => {
          ...
          window.history.pushState(null, "", `/search${next ? `?q=${encodeURIComponent(next)}` : ""}`);
