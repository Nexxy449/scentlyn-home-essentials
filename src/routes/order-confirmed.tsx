...
function OrderConfirmed() {
  ...
  useEffect(() => {
    const saved = readOrder(); setOrder(saved); setLoaded(true);
    if (!saved || saved.paymentMethod === "cod" || !paymentReference) return;
    let cancelled = false; let attempts = 0;
    const check = async () => {
      if (cancelled || attempts >= 10) return;
      attempts += 1; setChecking(true);
      try {
        const result = await verifyPaystackPayment({ data: { reference: paymentReference } });
        if (cancelled) return;
        if (result.status === "paid") { const next={...saved,paymentStatus:"paid" as const}; setOrder(next); try{window.sessionStorage.setItem("scentlyn-last-order",JSON.stringify(next));}catch{} setPaymentError(""); setChecking(false); return; }
        if (result.status === "failed") { const next={...saved,paymentStatus:"failed" as const}; setOrder(next); try{window.sessionStorage.setItem("scentlyn-last-order",JSON.stringify(next));}catch{} setPaymentError("Paystack did not confirm the payment. If you were charged, please contact Scentlyn before trying again."); setChecking(false); return; }
      } catch (error) { if (!cancelled) setPaymentError(error instanceof Error ? error.message : "We could not verify the payment yet."); }
      if (!cancelled) { setChecking(false); window.setTimeout(check,3000); }
    };
  ...
  const paid=order.paymentStatus==="paid"; const failed=order.paymentStatus==="failed"; const pendingPayment=order.paymentMethod!=="cod"&&!paid&&!failed;
